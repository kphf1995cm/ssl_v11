...
#define CENTER_SRVIP	"101.6.28.10"
...
void do_service(SSL *ssl, char *cliip)
{
	...
	while (1) {
		...
		char sid[16] = {0};
		char tel[16] = {0};
		int id_type = 0; /* client_type in radpostauth */
		char id_name[16] = {0}; /* client_name in radpostauth */
		char digest[64] = {0};
		char ip[64] = {0};
		char mytime[32] = {0};
		char mac[13] = {0};
		 ...
		 * Query	
		 * -----
		 * 	nid/sid/tel:xxx	<-> nid/sid/tel:xxx;ans:yes;nonce:xxx
		 * 					<-> nid/sid/tel:xxx;ans:no
		 * Auth
		 * ----
		 * 	nid:xxx;digest:xxx;mac:xxx <-> nid:xxx;auth:yes;hmac:xxx;idea_mac:xxx
		 * 							   <-> nid:xxx;auth:yes;idea_time:xxx
		 * 							   <-> nid:xxx;auth:no
		 * 	sid:xxx;digest:xxx;mac:xxx <-> sid:xxx;auth:yes;hmac:xxx;idea_mac:xxx;nid:xxx
		 * 							   <-> sid:xxx;auth:yes;idea_time:xxx;nid:xxx
		 * 							   <-> sid:xxx;auth:no
		 * 	tel:xxx;digest:xxx;mac:xxx <-> tel:xxx;auth:yes;hmac:xxx;idea_mac:xxx;nid:xxx
		 * 							   <-> tel:xxx;auth:yes;idea_time:xxx;nid:xxx
		 * 							   <-> tel:xxx;auth:no
		 * Msg
		 * ---
		 * 	nid/sid/tel:xxx;ip:xxx;time:xxx;mac:xxx
		 */
		/*
		 * id_type
		 * -------
		 ...
		 *  2 -> tel
		 */
		/* i. Query: nid/sid/tel:xxx */
		int query = 0;
		if (strstr(recvbuf.buf, ":") && !strchr(recvbuf.buf, ';')) { 
			srand((unsigned)time(NULL));
			int nonce = rand();
			if (strstr(recvbuf.buf, "nid:")) {
				strcpy(nid, strchr(recvbuf.buf, ':')+1);
				if (query_nid(nid)) { /* local nid found */
					query = 1;
			...
			} else if (strstr(recvbuf.buf, "sid:")) {
				strcpy(sid, strchr(recvbuf.buf, ':')+1); 
				get_nid(nid, sid);
				if (strlen(nid)) { /* local sid found */
					query = 1;
					sprintf(send_cont, "sid:%s;ans:yes;nonce:%d", sid, nonce); 
				} else sprintf(send_cont, "sid:%s;ans:no", sid); 
			} else if (strstr(recvbuf.buf, "tel:")) {
				strcpy(tel, strchr(recvbuf.buf, ':')+1); 
				get_nid(nid, tel);
				if (strlen(nid)) { /* local tel found */
					query = 1;
					sprintf(send_cont, "tel:%s;ans:yes;nonce:%d", tel, nonce); 
				} else sprintf(send_cont, "tel:%s;ans:no", tel); 
			} 
			if (query) {
		...
		/* ii. Auth: nid/sid/tel:xxx;digest:xxx;mac:xxx */
		} else if (strstr(recvbuf.buf, "digest:") && strstr(recvbuf.buf, "mac:")) {
			if (strstr(recvbuf.buf, "nid:"))		id_type = 0;
			else if (strstr(recvbuf.buf, "sid:"))	id_type = 1;
			else if (strstr(recvbuf.buf, "tel:"))	id_type = 2;

			char backup_for_forward[1024] = {0};
			strcpy(backup_for_forward, recvbuf.buf);
        	...
        	strcpy(mac, saveptr); 
			printf("[do_service|Auth]: nid = %s, sid = %s, tel = %s, digest = %s, mac = %s\n", nid, sid, tel, digest, mac);
			...
			for (travel = dlist_head(&nid_nonlocal); travel != NULL; travel = dlist_next(travel)) {
				...
				if (strcmp((const char *)dlist_data(travel), nid) == 0) {
					memset(nonlocal_result, 0, 1024);
					forward_recv2center(backup_for_forward, nonlocal_result);
					strcpy(send_cont, nonlocal_result);
			...
			if (auth_pass_local || auth_pass_nonlocal) {
				if (idea_mac(idea_key) && strlen(mac)) { /* idea_mac */
					...
					if (id_type == 0)
                    	...
                    	sprintf(send_cont, "sid:%s;auth:yes;hmac:%s;idea_mac:%s;nid:%s", sid, hmac, idea_key, nid);
					else if (id_type == 2) 
                    	sprintf(send_cont, "tel:%s;auth:yes;hmac:%s;idea_mac:%s;nid:%s", tel, hmac, idea_key, nid);
                    insert_NA_MACHASH(nid, mac, hmac);
                } else /* idea_time */
					if (id_type == 0)
                    	sprintf(send_cont, "nid:%s;auth:yes;idea_time:%s", nid, idea_key);
					else if (id_type == 1) 
                    	sprintf(send_cont, "sid:%s;auth:yes;idea_time:%s;nid:%s", sid, idea_key, nid);
					else if (id_type == 2) 
                    	sprintf(send_cont, "tel:%s;auth:yes;idea_time:%s;nid:%s", tel, idea_key, nid);
                printf("[do_service|Auth]: idea_key = %s\n", idea_key);
			} else 
    			if (id_type == 0)		sprintf(send_cont, "nid:%s;auth:no", nid);
				else if (id_type == 1)	sprintf(send_cont, "sid:%s;auth:no", sid);
				else if (id_type == 2)	sprintf(send_cont, "tel:%s;auth:no", tel);

		 /*	iii. nid/sid/tel:xxx;ip:xxx;time:xxx;mac:xxx */
		} else if (strstr(recvbuf.buf, "ip:") && strstr(recvbuf.buf, "time:") && strstr(recvbuf.buf, "mac:")) {
			if (strstr(recvbuf.buf, "nid:"))		id_type = 0; 
			else if (strstr(recvbuf.buf, "sid:"))	id_type = 1; 
			else if (strstr(recvbuf.buf, "tel:"))	id_type = 2; 
			...
			strtok_r(tmp[0], ":", &saveptr);
			strcpy(id_name, saveptr); 
			if (id_type == 0)
        		strcpy(nid, id_name); 
			else if (id_type == 1 || id_type == 2) 
				get_nid(nid, id_name);
			...
			for (travel = dlist_head(&nid_local); travel != NULL; travel = dlist_next(travel))
				if (strcmp((const char *)dlist_data(travel), nid) == 0) {
					insert_radpostauth(nid, ip, mytime, mac, id_name, id_type);
...
int main(int argc, char *argv[])
{
	...
	while (1) {
		...
		while ((nread = getline(&line, &len, stream)) != -1) {
			//printf("Retrieved line of length %zu: ", nread);
